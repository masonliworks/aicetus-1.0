        ...
        VStack(spacing: 0) {
            ...
            if !bridge.pendingApprovals.isEmpty {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    ForEach(bridge.pendingApprovals) { session in
                        Button {
                            openWindow(id: "main")
                        } label: {
                            HStack(spacing: 8) {
                                ...
                                Text("\(session.title) · \(session.detail.replacingOccurrences(of: "等待 iPhone 审批 ", with: ""))")
                                    .font(.system(size: 13))
                                    .lineLimit(1)
                                Spacer()
                                if let since = session.waitingSince {
                                    Text(since)
